--DROP FUNCTION single_objects_linear_element(text, integer);
CREATE OR REPLACE FUNCTION single_objects_linear_element(dbschema text, bfsnr integer)
  RETURNS boolean AS
$$
BEGIN
  EXECUTE
'
WITH single_objects_linear_element AS (
...
FROM av_avdpool_ng.einzelobjekte_eonachfuehrung as a, av_avdpool_ng.einzelobjekte_einzelobjekt as b, av_avdpool_ng.einzelobjekte_linienelement as c
WHERE a.gem_bfs = '||bfsnr||'
AND b.gem_bfs = '||bfsnr||'
AND c.gem_bfs = '||bfsnr||'
AND a.t_id = b.entstehung
AND b.t_id = c.linienelement_von
),

foo AS (
 INSERT INTO av_mopublic_export.single_objects_linear_element (t_id, validity, type, quality, state_of, fosnr, geometry)
...
single_objects_linear_element_postext AS (
 ...
 FROM av_avdpool_ng.einzelobjekte_objektname as a, av_avdpool_ng.einzelobjekte_objektnamepos as b, single_objects_linear_element as c
 WHERE a.gem_bfs = '||bfsnr||'
 AND b.gem_bfs = '||bfsnr||'
...
FROM single_objects_linear_element_postext;
';
